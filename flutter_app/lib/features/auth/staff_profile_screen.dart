...
class _ProfileBodyState extends ConsumerState<_ProfileBody> {
  ...
  Future<void> _pickAndUploadAvatar() async {
    AppLogger.info('StaffProfileScreen', '🎤 AVATAR UPLOAD STARTED');
    ...
    if (picked == null) {
      AppLogger.info('StaffProfileScreen', '❌ Image pick was cancelled');
      return;
    }

    setState(() => _uploadingAvatar = true);
    try {
      AppLogger.info('StaffProfileScreen', '📸 Image selected: ${picked.path}');
      final bytes = await picked.readAsBytes();
      AppLogger.info('StaffProfileScreen', '📦 Image size: ${bytes.length} bytes');
      
      final staffId = widget.staff.id;
      final path = '$staffId.jpg';
      final client = Supabase.instance.client;

      AppLogger.info('StaffProfileScreen', '⬆️ Uploading to staff-avatars/$path');
      ...
      AppLogger.info('StaffProfileScreen', '✅ Upload successful to Supabase Storage');

      final url = client.storage.from('staff-avatars').getPublicUrl(path);
      AppLogger.info('StaffProfileScreen', '🔗 Public URL: $url');
      
      // Bust cache by appending a timestamp query param
      final bustedUrl = '$url?t=${DateTime.now().millisecondsSinceEpoch}';
      AppLogger.info('StaffProfileScreen', '💾 Updating database with URL: $bustedUrl');

      await ref
          .read(authServiceProvider)
          .updateStaffUser(staffId, avatarUrl: bustedUrl);
      AppLogger.info('StaffProfileScreen', '✅ Database updated');

      await ref.read(currentStaffProvider.notifier).refreshCurrentUser();
      AppLogger.info('StaffProfileScreen', '✅ User session refreshed');
